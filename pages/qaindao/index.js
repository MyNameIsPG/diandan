...
Page({
  ...
  onClickToDetail: function (e) {
    ...
    // var rule_desc = e.currentTarget.dataset.rule_desc
    var pos=this.data.postdata
    pos.mendian_id = e.currentTarget.dataset.mid
    pos.shanghu_id = e.currentTarget.dataset.sid
    $config.post('mendian/get_mendianuid', pos, function (data) {
      console.log(data)
        if(data.code==1){
          ...
          wx.navigateTo({
            url: '/pages/qaindao/detail?jsonStr=' + encodeURIComponent(str),
            success: function (re) {
              // success 
            },
          })
